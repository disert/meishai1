package com.meishai.lib.photoview.gif;
...
public class GifDrawable extends Drawable implements Animatable, MediaPlayerControl {
     ...
     * Buffer can be larger than size of the GIF data. Bytes beyond GIF terminator are not
     * accessed.
     *
    ...
    public String toString() {
        return String
                .format(Locale.US, "Size: %dx%d, %d frames, error: %d", mMetaData[0], mMetaData[1],
     ...
     *
     * @param factor new speed factor, eg. 0.5f means half speed, 1.0f - normal, 2.0f - double
     *               speed
     ...
     * For GifDrawables constructed from {@link InputStream} and {@link FileDescriptor} -1 is always
     * returned.
     * In case of {@link File}, file path, byte array and {@link ByteBuffer} length is always
     * known.
     *
    ...
    public void getPixels(int[] pixels) {
        ...
        if (pixels.length < colors.length) {
            throw new ArrayIndexOutOfBoundsException(
                    "Pixels array is too small. Required length: " + colors.length);
        }
     ...
     * @throws IllegalArgumentException if x, y exceed the drawable's bounds or drawable is
     *                                  recycled
     */
    ...
    public void draw(Canvas canvas) {
        ...
        if (mPaint.getShader() == null) {
            ...
            final int[] colors = mColors;
            if (colors != null) {
                canvas.drawBitmap(colors, 0, mMetaData[0], 0f, 0f, mMetaData[0], mMetaData[1], true,
                        mPaint);
            }

            if (mMetaData[4] >= 0 && mMetaData[2] > 1) {
                UI_HANDLER.postDelayed(mInvalidateTask,
                        mMetaData[4]);//TODO don't post if message for given frame was already posted
            }
        } else {
