...
public class WheelView extends View {
    /**
     * Scrolling duration 滑动持续时间
     */
    private static final int SCROLLING_DURATION = 400;

    /**
     * Minimum delta for scrolling 滑动最小增量
     */
	 ...
	 */
    /**
     * Top and bottom shadows colors 渐变颜色
     */
    ...
    public WheelView(Context context, AttributeSet attrs, int defStyle) {
        ...
        itemHeight = DensityUtils.dp2px(context, 40);
        initData(context);
    }
     ...
     * @param adapter
     */
    public void setShiAdapter(WheelAdapter adapter) {
        currentItem = 0;
        this.adapter = adapter;
        invalidateLayouts();
        invalidate();
    ...
    private void initResourcesIfNecessary() {
        ...
        if (valuePaint == null) {
            ...
            valuePaint.setShadowLayer(0.1f, 0, 0.1f, 0xFFFFFFFF);
        }
    ...
    private int calculateLayoutWidth(int widthSize, int mode) {
        ...
        if (maxLength > 0) {
            float textWidth = FloatMath.ceil(Layout.getDesiredWidth("汉",
                    itemsPaint));
            itemsWidth = (int) (maxLength * (textWidth));
        } else {
            itemsWidth = 0;
        }
        itemsWidth += (ADDITIONAL_ITEMS_SPACE * 2); // make it some more
