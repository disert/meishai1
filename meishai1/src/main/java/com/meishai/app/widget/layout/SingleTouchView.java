...
public class SingleTouchView extends View {
    ...
    private void obtainStyledAttributes(AttributeSet attrs) {
        ...
        frameColor = mTypedArray.getColor(R.styleable.SingleTouchView_frameColor1, DEFAULT_FRAME_COLOR);
    ...
    private void init() {
        ...
        if (controlDrawable == null) {//设置控制点的默认
            controlDrawable = getContext().getResources().getDrawable(R.drawable.ic_sticker_control);
        }
        if (deleteDrawable == null) {//设置删除点的默认
            deleteDrawable = getContext().getResources().getDrawable(R.drawable.ic_sticker_delete);
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (null != mViewGroup) {
            ...
            DebugLog.w("parentWidth:" + parentWidth + ",parentHeight:" + parentHeight);
            DebugLog.w("mCenterPoint:" + mCenterPoint.x + "," + mCenterPoint.y);
    ...
    public Bundle saveState() {
        ...
        bundle.putInt("isVisibility", getVisibility());//用来判断是否需要显示的
        bundle.putInt("framePadding", framePadding);
        ...
        bundle.putInt("mDeleteWidth", mDeleteWidth);
        bundle.putInt("mDeleteHeight", mDeleteHeight);
        bundle.putFloat("mDegree", mDegree);
        bundle.putFloat("mScale", mScale);
        bundle.putInt("mDrawableWidth", mDrawableWidth);
        bundle.putInt("mDrawableHeight", mDrawableHeight);
        bundle.putInt("mViewPaddingLeft", mViewPaddingLeft);
        bundle.putInt("mViewPaddingTop", mViewPaddingTop);
        bundle.putParcelable("mCenterPoint", mCenterPoint);
        bundle.putParcelable("mControlPoint", mControlPoint);
        bundle.putParcelable("mDeletePoint", mDeletePoint);
        bundle.putParcelable("mLBPoint", mLBPoint);
        bundle.putParcelable("mLTPoint", mLTPoint);
        bundle.putParcelable("mRBPoint", mRBPoint);
        bundle.putParcelable("mRTPoint", mRTPoint);
        bundle.putParcelable("mPreMovePointF", mPreMovePointF);
        bundle.putParcelable("mCurMovePointF", mCurMovePointF);
    ...
    public void resetState(Bundle bundle) {
        ...
        framePadding = bundle.getInt("framePadding");
        mViewHeight = bundle.getInt("mViewHeight");
        mViewWidth = bundle.getInt("mViewWidth");
        offsetX = bundle.getInt("offsetX");
        offsetY = bundle.getInt("offsetY");
        mDeleteWidth = bundle.getInt("mDeleteWidth");
        mDeleteHeight = bundle.getInt("mDeleteHeight");
        mDegree = bundle.getFloat("mDegree");
        mScale = bundle.getFloat("mScale");
        mDrawableWidth = bundle.getInt("mDrawableWidth");
        mDrawableHeight = bundle.getInt("mDrawableHeight");
        mViewPaddingLeft = bundle.getInt("mViewPaddingLeft");
        mViewPaddingTop = bundle.getInt("mViewPaddingTop");
        mCenterPoint = bundle.getParcelable("mCenterPoint");
        mControlPoint = bundle.getParcelable("mControlPoint");
        mDeletePoint = bundle.getParcelable("mDeletePoint");
        mLBPoint = bundle.getParcelable("mLBPoint");
        mLTPoint = bundle.getParcelable("mLTPoint");
        mRBPoint = bundle.getParcelable("mRBPoint");
        mRTPoint = bundle.getParcelable("mRTPoint");
        mPreMovePointF = bundle.getParcelable("mPreMovePointF");
        mCurMovePointF = bundle.getParcelable("mCurMovePointF");
        invalidate();
//        adjustLayout();
    }
            ...
            case MotionEvent.ACTION_UP:
                if (mStatus == STATUS_DELETE) {
                    ...
                    if ((upTime - downTime) < 1000) {
                        if (JudgeStatus(event.getX(), event.getY()) == STATUS_DELETE) {
                            isEditable = false;
    ...
    /**
     * 获取到贴纸图片的左上角的位置,用于在编辑完成之后把贴纸画到图片上
     *
     * @return
     */
    public Point getLTPoint() {
        int actualWidth = mViewWidth;
        int actualHeight = mViewHeight;

        int newPaddingLeft = (int) (mCenterPoint.x - actualWidth / 2);
        int newPaddingTop = (int) (mCenterPoint.y - actualHeight / 2);
        return new Point(newPaddingLeft, newPaddingTop);
    }
