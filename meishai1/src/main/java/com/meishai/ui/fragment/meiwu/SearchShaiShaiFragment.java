 ...
 * @author yl
 */
public class SearchShaiShaiFragment extends BaseFragment {
    private View view;
    ...
    protected void getRequestData(final String key) {
        DebugLog.w("SearchShaiShaiFragment 的 getRequestData");
        if (TextUtils.isEmpty(key) || !isInit) {
            return;
        }
        MeiWuReq.search(key, 1, new Listener<String>() {
    ...
    protected void checkRespData(String response) {
        try {
            ...
            if (success == 1) {
                TopicRespData resqData = GsonHelper.parseObject(response,
                        TopicRespData.class);
                mData = resqData;
    ...
    public void setKey(String key) {
        DebugLog.w("当前fragment:SearchShaiShaiFragment");
